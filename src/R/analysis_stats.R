...
packages <- c("jsonlite", "tidyverse", "lubridate", "forecast",
              ...
              "randomForest", "caret", "DBI", "ROracle")
...
get_data_from_db <- function(connection_string) {
  con <- dbConnect(Oracle(), connection_string)
  query <- "
    SELECT l.timestamp, l.temp, l.humid, l.P, l.K, l.pH, l.estado_irrigacao,
           s.tipo as sensor_tipo, a.end_localizacao, t.nome as cultura_nome
    FROM leituras l
    JOIN sensor s ON l.id_sensor = s.id_sensor
    JOIN area_cultivo a ON s.id_area = a.id_area
    JOIN tipo_cultura t ON a.id_cultura = t.id_cultura
    ORDER BY l.timestamp
  "
  leituras <- dbGetQuery(con, query)
  dbDisconnect(con)
  ...
  leituras %>%
    mutate(
      ...
      P = factor(P),
      K = factor(K),
      irrigacao_status = factor(estado_irrigacao),
      irrigacao_num = ifelse(irrigacao_status == "ligada", 1, 0)
    )
}

# Função principal para executar a análise
run_analysis <- function(connection_string) {
  leituras <- get_data_from_db(connection_string)
